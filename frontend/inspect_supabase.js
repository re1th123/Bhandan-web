const SUPABASE_REST_URL = 'https://wwxicktggkobdyuvgilb.supabase.co/rest/v1';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind3eGlja3RnZ2tvYmR5dXZnaWxiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ5Mzc2MzAsImV4cCI6MjEwMDUxMzYzMH0.4SvbnxCXzjwdg9auN_7hVMABCmj8-c5a8mCkZc69MHc';

const headers = {
  'apikey': SUPABASE_ANON_KEY,
  'Authorization': `Bearer ${SUPABASE_ANON_KEY}`
};

async function inspectTable(table) {
  const res = await fetch(`${SUPABASE_REST_URL}/${table}?select=*&limit=1`, { headers });
  console.log(`Table '${table}' Status:`, res.status);
  if (res.ok) {
    const data = await res.json();
    console.log(`Table '${table}' Data:`, data);
  } else {
    const text = await res.text();
    console.log(`Table '${table}' Error:`, text);
  }
}

async function run() {
  await inspectTable('businesses');
  await inspectTable('customers');
  await inspectTable('suppliers');
  await inspectTable('products');
  await inspectTable('warehouses');
  await inspectTable('invoices');
  await inspectTable('purchases');
  await inspectTable('journal_entries');
}

run();
